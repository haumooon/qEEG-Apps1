#!/usr/bin/env python3
# qEEG Report: Depression vs Bipolar (weighted markers + percentages)
...
# Regions for alpha reduction rule
REGIONS = {
    "Central":  ["C3","C4","Cz"],
...
def normalize_label(ch: str) -> str:
    ...
    # Map older names to modern system
    mapping = {
        "FP1":"Fp1", "FP2":"Fp2",
...
def check_channel_mapping(raw, norms):
    """Return list of (original_label, normalized_label) that are not in Cuban norms."""
...
def _epochs_psd_and_freqs(epochs, fmin=1., fmax=30.):
    """Return (psds, freqs) with shape (n_epochs, n_channels, n_freqs) if possible."""
...
def compute_raw_powers(edf_path: str) -> pd.DataFrame:
    """Band powers from RAW (no artifact rejection)."""
...
def compute_clean_powers(edf_path: str, p2p_uv: float = 150.0) -> tuple[pd.DataFrame,int]:
    """Band powers from artifact-rejected epochs."""
    raw = mne.io.read_raw_edf(edf_path, preload=True, verbose=False)
    ...
    if len(epochs)==0:
        # Relax threshold once if everything got rejected
        epochs = mne.Epochs(raw, events=events, tmin=0., tmax=2.0,
                            baseline=None, preload=True, verbose=False)
...
def add_zrel_zabs(df: pd.DataFrame, norms_age: pd.DataFrame) -> pd.DataFrame:
    ...
    def z_abs(v,m,s): return (np.log10(v)+12.0-m)/s if (v>0 and np.isfinite(v)) else np.nan  # back to µV^2
    out = df.copy()
    for b in BANDS:
        zr, za = [], []
...
def bipolar_theta_signature(dfz: pd.DataFrame):
    # Right-lower chain: T8, P8, P4, O2; require >= 2 sites with Zrel_Theta >= +1.0
...
def region_alpha_reduced(dfz: pd.DataFrame, chans: list[str]):
...
def bipolar_alpha_signature(dfz: pd.DataFrame):
    # Reduced if Central & Parietal OR Parietal & Occipital are reduced
...
def main():
    ...
    try:
        ...
        # ---------------- Diagnostics & Weighted Scoring ----------------
        doc.add_heading("Research-Based Diagnostics", level=1)

        # FAA (Depression marker: left-dominant alpha)
        ...
        # Parietal Alpha Asymmetry (P3 > P4) – Depression marker
        ...
        # APF Slowing at Cz (<= 9.5 Hz adults) – Depression marker
        cz_pick = None
        ...
        # Alpha Reduction (Central&Parietal OR Parietal&Occipital) – Bipolar marker
        ...
        # Theta right-lower chain – Bipolar marker
        ...
        # Parietal Beta Bilateral – Bipolar marker
        ...
        # ---- Final weighted scoring -> percentages ----
        dep_score = faa_points + parietal_alpha_points + apf_points                  # max 6
        bp_score  = alpha_reduction_points + theta_points + parietal_beta_points    # max 6
        total = dep_score + bp_score
        if total > 0:
            p_dep = dep_score / total
            p_bp  = bp_score  / total
        else:
            p_dep = p_bp = 0.5  # fallback
